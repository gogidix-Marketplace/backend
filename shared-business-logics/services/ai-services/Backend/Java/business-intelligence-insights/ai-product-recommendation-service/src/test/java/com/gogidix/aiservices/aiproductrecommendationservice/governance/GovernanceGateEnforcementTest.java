package com.gogidix.aiservices.aiproductrecommendationservice.governance;

import com.gogidix.aiservices.aiproductrecommendationservice.infrastructure.governance.ThresholdValidator;
...
class GovernanceGateEnforcementTest {
    ...
    class P95LatencyGateTests {
        ...
        void shouldPassP95LatencyGateWhenUnderThreshold() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordRecommendationTime(300 + (i * 3)); // Max ~450ms
            }
        ...
        void shouldFailP95LatencyGateWhenOverThreshold() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordRecommendationTime(400 + (i * 5)); // Max ~650ms
            }
    ...
    @Nested
    @DisplayName("Recommendation Analysis Latency Gate")
    class RecommendationAnalysisLatencyGateTests {

        @Test
        @DisplayName("Should pass segment analysis latency gate when under threshold")
        void shouldPassRecommendationAnalysisLatencyGateWhenUnderThreshold() {
            // Record analysis latencies under 200ms threshold
            for (int i = 0; i < 30; i++) {
                var sample = metrics.startRecommendationAnalysisTimer();
                try {
    ...
    class OverallHealthStatusGateTests {
        ...
        void shouldReportHealthyStatusWhenAllGatesPass() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordRecommendationTime(300); // Under P95 threshold
            }
        ...
        void shouldReportDegradedStatusWhenSomeGatesFail() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordRecommendationTime(300);
            }
