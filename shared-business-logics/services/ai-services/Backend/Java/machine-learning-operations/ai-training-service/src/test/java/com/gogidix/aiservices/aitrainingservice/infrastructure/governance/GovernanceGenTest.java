...
class GovernanceGenTest {
    ...
    @Test
    void validate_P95_JOB_LATENCY() {
        var result = validator.validateThreshold(ThresholdValidator.ThresholdType.P95_JOB_LATENCY);
        assertThat(result).isNotNull();
        assertThat(result.isPassed()).isTrue();
    }

    @Test
    void validate_P99_JOB_LATENCY() {
        var result = validator.validateThreshold(ThresholdValidator.ThresholdType.P99_JOB_LATENCY);
        assertThat(result).isNotNull();
        assertThat(result.isPassed()).isTrue();
    }

    @Test
    void validate_MODEL_TRAINING_LATENCY() {
        var result = validator.validateThreshold(ThresholdValidator.ThresholdType.MODEL_TRAINING_LATENCY);
        assertThat(result).isNotNull();
        assertThat(result.isPassed()).isTrue();
    }
