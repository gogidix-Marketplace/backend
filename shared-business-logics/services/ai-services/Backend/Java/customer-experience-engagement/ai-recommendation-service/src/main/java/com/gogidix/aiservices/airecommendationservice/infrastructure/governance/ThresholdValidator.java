...
public class ThresholdValidator {
    ...
    // Threshold definitions for Customer Experience - Recommendation Service
    ...
    private static final double MAX_ML_LATENCY_MS = 200.0;

    public ThresholdValidator(RecommendationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double mlLatency = metrics.getMlPredictionLatencyP95();
        boolean mlCompliant = mlLatency <= MAX_ML_LATENCY_MS;
        report.addCheck("ML Prediction Latency", mlLatency, MAX_ML_LATENCY_MS, mlCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getRecommendationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getRecommendationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case ML_PREDICTION_LATENCY -> {
                double value = metrics.getMlPredictionLatencyP95();
                boolean passed = value <= MAX_ML_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_ML_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
