...
public class ComplianceReportGenerator {
    ...
    private final BusinessAutomationMetrics metrics;
    private final ThresholdValidator thresholdValidator;

    // SLO Targets for Business Operations
    ...
    public ComplianceReportGenerator(
            BusinessAutomationMetrics metrics,
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.business.automation.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.business.automation.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.business.automation.failure").counter().count());

        section.setWorkflowsExecuted((long) metrics.getMeterRegistry()
                .get("ai.business.automation.workflow.executed").counter().count());

        section.setApprovalsProcessed((long) metrics.getMeterRegistry()
                .get("ai.business.automation.approval.processed").counter().count());

        section.setDocumentsProcessed((long) metrics.getMeterRegistry()
                .get("ai.business.automation.document.processed").counter().count());

        section.setNotificationsSent((long) metrics.getMeterRegistry()
                .get("ai.business.automation.notification.sent").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("workflowsExecuted", performanceMetrics.getWorkflowsExecuted());
            map.put("approvalsProcessed", performanceMetrics.getApprovalsProcessed());
            map.put("documentsProcessed", performanceMetrics.getDocumentsProcessed());
            map.put("notificationsSent", performanceMetrics.getNotificationsSent());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long approvalsProcessed;
        ...
        public long getWorkflowsExecuted() { return workflowsExecuted; }
        public void setWorkflowsExecuted(long workflowsExecuted) { this.workflowsExecuted = workflowsExecuted; }
        public long getApprovalsProcessed() { return approvalsProcessed; }
        public void setApprovalsProcessed(long approvalsProcessed) { this.approvalsProcessed = approvalsProcessed; }
