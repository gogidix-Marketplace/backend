package com.gogidix.aiservices.aiorchestrationservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private final AiOrchestrationMetrics metrics;
    ...
    public ComplianceReportGenerator(
            AiOrchestrationMetrics metrics,
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P99 Latency
        double p99Latency = metrics.getOrchestrationLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.orchestration.requests.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.orchestration.requests.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.orchestration.requests.failure").counter().count());

        section.setWorkflowsExecuted((long) metrics.getMeterRegistry()
                .get("ai.orchestration.workflows.executed").counter().count());

        section.setStepsExecuted((long) metrics.getMeterRegistry()
                .get("ai.orchestration.steps.executed").counter().count());
    ...
    public static class PerformanceMetricsSection {
        ...
        public void setStepsExecuted(long stepsExecuted) { this.stepsExecuted = stepsExecuted; }
