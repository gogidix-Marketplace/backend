...
# Development utility script for centralized-performance-metrics
...
start_service() {
    echo "Starting $centralized-performance-metrics..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${centralized-performance-metrics}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$centralized-performance-metrics started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $centralized-performance-metrics..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$centralized-performance-metrics stopped"
        else
            echo "$centralized-performance-metrics is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $centralized-performance-metrics may not be running"
    fi
...
run_tests() {
    echo "Running tests for $centralized-performance-metrics..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $centralized-performance-metrics in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $centralized-performance-metrics health..."
...
service_info() {
    ...
    echo "==================="
    echo "Name: $centralized-performance-metrics"
