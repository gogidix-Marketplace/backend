package com.gogidix.aiservices.aichurnpredictionservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private final ChurnPredictionMetrics metrics;
    ...
    public ComplianceReportGenerator(
            ChurnPredictionMetrics metrics,
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalPredictions((long) metrics.getMeterRegistry()
                .get("segmentation.total").counter().count());

        section.setSuccessfulPredictions((long) metrics.getMeterRegistry()
                .get("segmentation.success").counter().count());

        section.setFailedPredictions((long) metrics.getMeterRegistry()
                .get("segmentation.failure").counter().count());

        section.setPredictionsCreated((long) metrics.getMeterRegistry()
                .get("segment.created").counter().count());

        section.setPredictionsUpdated((long) metrics.getMeterRegistry()
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            Map<String, Object> map = new HashMap<>();
            map.put("totalPredictions", performanceMetrics.getTotalPredictions());
            map.put("successfulPredictions", performanceMetrics.getSuccessfulPredictions());
            map.put("failedPredictions", performanceMetrics.getFailedPredictions());
            map.put("segmentsCreated", performanceMetrics.getPredictionsCreated());
    ...
    public static class PerformanceMetricsSection {
        ...
        public void setSuccessfulPredictions(long successfulPredictions) { this.successfulPredictions = successfulPredictions; }
        public long getFailedPredictions() { return failedPredictions; }
        public void setFailedPredictions(long failedPredictions) { this.failedPredictions = failedPredictions; }
        public long getPredictionsCreated() { return segmentsCreated; }
        public void setPredictionsCreated(long segmentsCreated) { this.segmentsCreated = segmentsCreated; }
        public long getPredictionsUpdated() { return segmentsUpdated; }
        public void setPredictionsUpdated(long segmentsUpdated) { this.segmentsUpdated = segmentsUpdated; }
