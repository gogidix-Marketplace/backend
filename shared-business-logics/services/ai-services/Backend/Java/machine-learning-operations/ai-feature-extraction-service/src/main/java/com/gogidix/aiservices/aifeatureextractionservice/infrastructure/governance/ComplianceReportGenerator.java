...
public class ComplianceReportGenerator {
    ...
    private final FeatureExtractionMetrics metrics;
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.feature.extraction.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.feature.extraction.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.feature.extraction.failure").counter().count());

        section.setFeaturesExtracted((long) metrics.getMeterRegistry()
                .get("ai.feature.extracted").counter().count());

        section.setFeatureSetsProcessed((long) metrics.getMeterRegistry()
                .get("ai.feature.set.processed").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("featuresExtracted", performanceMetrics.getFeaturesExtracted());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long featuresExtracted;
        private long featureSetsProcessed;
        ...
        public long getFeaturesExtracted() { return featuresExtracted; }
        public void setFeaturesExtracted(long featuresExtracted) { this.featuresExtracted = featuresExtracted; }
        public long getFeatureSetsProcessed() { return featureSetsProcessed; }
        public void setFeatureSetsProcessed(long featureSetsProcessed) { this.featureSetsProcessed = featureSetsProcessed; }
