package com.gogidix.aiservices.aivoiceassistantservice.infrastructure.governance;

import com.gogidix.aiservices.aivoiceassistantservice.infrastructure.metrics.VoiceAssistantMetrics;
...
public class ThresholdValidator {
    ...
    private final VoiceAssistantMetrics metrics;
    ...
    private static final double MAX_STT_LATENCY_MS = 400.0;

    public ThresholdValidator(VoiceAssistantMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double p95Latency = metrics.getAssistantLatencyP95();
        boolean p95Compliant = p95Latency <= MAX_P95_LATENCY_MS;
        report.addCheck("P95 Latency", p95Latency, MAX_P95_LATENCY_MS, p95Compliant);

        double p99Latency = metrics.getAssistantLatencyP99();
        boolean p99Compliant = p99Latency <= MAX_P99_LATENCY_MS;
        report.addCheck("P99 Latency", p99Latency, MAX_P99_LATENCY_MS, p99Compliant);

        double sttLatency = metrics.getSpeechToTextLatencyP95();
        boolean sttCompliant = sttLatency <= MAX_STT_LATENCY_MS;
        report.addCheck("Speech-to-Text Latency", sttLatency, MAX_STT_LATENCY_MS, sttCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getAssistantLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getAssistantLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case SPEECH_TO_TEXT_LATENCY -> {
                double value = metrics.getSpeechToTextLatencyP95();
                boolean passed = value <= MAX_STT_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_STT_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
