package com.gogidix.aiservices.aimanagementservice.infrastructure.governance;
...
public class ThresholdValidator {
    ...
    private final ModelManagementMetrics metrics;

    // Threshold definitions
    private static final double MAX_P95_REGISTRATION_LATENCY_MS = 500.0;
    private static final double MAX_P99_REGISTRATION_LATENCY_MS = 1000.0;
    private static final double MAX_ERROR_RATE = 0.01;  // 1%
    private static final double MAX_MODEL_LOADING_LATENCY_MS = 5000.0;

    public ThresholdValidator(ModelManagementMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate P95 registration latency
        double p95Latency = metrics.getModelRegistrationLatencyP95();
        boolean p95Compliant = p95Latency <= MAX_P95_REGISTRATION_LATENCY_MS;
        report.addCheck("P95 Registration Latency", p95Latency, MAX_P95_REGISTRATION_LATENCY_MS, p95Compliant);

        // Validate P99 registration latency
        double p99Latency = metrics.getModelRegistrationLatencyP99();
        boolean p99Compliant = p99Latency <= MAX_P99_REGISTRATION_LATENCY_MS;
        report.addCheck("P99 Registration Latency", p99Latency, MAX_P99_REGISTRATION_LATENCY_MS, p99Compliant);

        // Validate model loading latency
        double loadingLatency = metrics.getModelLoadingLatencyP95();
        boolean loadingCompliant = loadingLatency <= MAX_MODEL_LOADING_LATENCY_MS;
        report.addCheck("Model Loading Latency", loadingLatency, MAX_MODEL_LOADING_LATENCY_MS, loadingCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_REGISTRATION_LATENCY -> {
                double value = metrics.getModelRegistrationLatencyP95();
                boolean passed = value <= MAX_P95_REGISTRATION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_REGISTRATION_LATENCY_MS, "ms", passed);
            }
            case P99_REGISTRATION_LATENCY -> {
                double value = metrics.getModelRegistrationLatencyP99();
                boolean passed = value <= MAX_P99_REGISTRATION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_REGISTRATION_LATENCY_MS, "ms", passed);
            }
            case MODEL_LOADING_LATENCY -> {
                double value = metrics.getModelLoadingLatencyP95();
                boolean passed = value <= MAX_MODEL_LOADING_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_MODEL_LOADING_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
    ...
    public enum ThresholdType {
        P95_REGISTRATION_LATENCY,
        P99_REGISTRATION_LATENCY,
