...
public class ComplianceReportGenerator {
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P99 Latency
        double p99Latency = metrics.getInferenceLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.inference.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.inference.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.inference.failure").counter().count());

        section.setModelsLoaded((long) metrics.getMeterRegistry()
                .get("ai.model.loaded").counter().count());

        section.setHighConfidenceInferences((long) metrics.getMeterRegistry()
                .get("ai.inference.high.confidence").counter().count());

        section.setLowConfidenceInferences((long) metrics.getMeterRegistry()
                .get("ai.inference.low.confidence").counter().count());
    ...
    public static class PerformanceMetricsSection {
        ...
        private long highConfidenceInferences;
        private long lowConfidenceInferences;
        ...
        public long getHighConfidenceInferences() { return highConfidenceInferences; }
        public void setHighConfidenceInferences(long highConfidenceInferences) { this.highConfidenceInferences = highConfidenceInferences; }
        public long getLowConfidenceInferences() { return lowConfidenceInferences; }
        public void setLowConfidenceInferences(long lowConfidenceInferences) { this.lowConfidenceInferences = lowConfidenceInferences; }
