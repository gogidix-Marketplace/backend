# AI User Profileation Service - Blueprint Certification Report
...
```
com.gogidix.aiservices.aiuserprofilingservice
├── domain/
│   ├── model/          ← UserProfile, ProfileCriteria, ProfileType, ProfileStatus
│   ├── repository/      ← UserProfileRepository interface
│   ├── policy/         ← ProfileBusinessPolicy, UserProfilePolicy
│   └── port/
│       ├── in/        ← UserProfileServicePort (use application DTOs)
│       └── out/       ← UserProfileRepositoryPort (implemented by infrastructure)
├── application/
│   ├── command/        ← Create, Update, AddUsers, RemoveUsers, Analyze commands
│   ├── dto/           ← UserProfileResponseDto, PagedResponseDto, ProfileAnalysisResponseDto
│   ├── query/          ← FindProfilesByTenantQuery
│   └── service/        ← UserProfileApplicationService (orchestrates use cases)
├── infrastructure/
│   ├── persistence/     ← UserProfileRepositoryAdapter, UserProfileDocument, SpringDataUserProfileRepository
│   ├── messaging/       ← ProfileEventPublisher (Kafka)
│   └── security/        ← JwtTokenProvider, JwtAuthenticationFilter, SecurityConfiguration
├── interfaces/
│   └── rest/          ← UserProfileController, RestExceptionHandler
└── shared/
...
| Feature | Description |
|----------|-------------|
| **UserProfile** | Aggregate root with lifecycle management (DRAFT → ACTIVE → INACTIVE → ARCHIVED) |
| **ProfileCriteria** | Value object with builder pattern, supports nested criteria with logical operators |
| **ProfileBusinessPolicy** | Business rules for segment activation, customer count updates, criteria validation |
| **UserProfilePolicy** | Domain policy interface defining segment business rules |
| **Repository Ports** | UserProfileRepositoryPort (domain contract) |

### Application Layer
| Feature | Description |
|----------|-------------|
| **Commands** | CreateProfileCommand, UpdateProfileCommand, AddUsersToProfileCommand, RemoveUsersFromProfileCommand, AnalyzeProfileCommand |
| **DTOs** | UserProfileResponseDto (immutable), PagedResponseDto, ProfileAnalysisResponseDto |
| **Queries** | FindProfilesByTenantQuery (pagination, filtering by type/active) |
| **Service** | UserProfileApplicationService - orchestrates all use cases, delegates to domain |
...
| **Messaging** | Spring Kafka | `ProfileEventPublisher` publishes `ProfileCreatedEvent`, `ProfileUpdatedEvent`, `ProfileDeletedEvent` |
...
|---------|-------------|-------|--------|
| Domain | UserProfileTest | 27 | ✅ |
| Domain | ProfileCriteriaTest | 16 | ✅ |
| Application | CreateProfileCommandTest | 14 | ✅ |
| Application | AddUsersToProfileCommandTest | 11 | ✅ |
| Application | UpdateProfileCommandTest | 9 | ✅ |
| Application | FindProfilesByTenantQueryTest | 8 | ✅ |
...
| Test | Status |
|------|--------|
| UserProfileIntegrationTest | Requires Docker/Testcontainers (skipped locally) |
...
spring:
  application:
    name: AI User Profileation Service
  data:
    mongodb:
...
mv $SERVICE_NAME/src/main/java/com/gogidix/aiservices/aiuserprofilingservice/AIUserProfileationServiceApplication.java \
      $SERVICE_NAME/src/main/java/com/gogidix/aiservices/aiuserprofilingservice/AI${SERVICE_NAME#ai-}Application.java
