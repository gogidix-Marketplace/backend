...
public class ThresholdValidator {
    ...
    // Threshold definitions for Business Operations
    ...
    private static final double MAX_WORKFLOW_LATENCY_MS = 750.0;
    private static final double MAX_AI_DECISION_LATENCY_MS = 200.0;

    public ThresholdValidator(BusinessAutomationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate workflow execution latency
        double workflowLatency = metrics.getWorkflowExecutionLatencyP95();
        boolean workflowCompliant = workflowLatency <= MAX_WORKFLOW_LATENCY_MS;
        report.addCheck("Workflow Execution Latency", workflowLatency, MAX_WORKFLOW_LATENCY_MS, workflowCompliant);

        // Validate AI decision latency
        double aiDecisionLatency = metrics.getAiDecisionLatencyP95();
        boolean aiDecisionCompliant = aiDecisionLatency <= MAX_AI_DECISION_LATENCY_MS;
        report.addCheck("AI Decision Latency", aiDecisionLatency, MAX_AI_DECISION_LATENCY_MS, aiDecisionCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getAutomationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getAutomationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case WORKFLOW_EXECUTION_LATENCY -> {
                double value = metrics.getWorkflowExecutionLatencyP95();
                boolean passed = value <= MAX_WORKFLOW_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_WORKFLOW_LATENCY_MS, "ms", passed);
            }
            case AI_DECISION_LATENCY -> {
                double value = metrics.getAiDecisionLatencyP95();
                boolean passed = value <= MAX_AI_DECISION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_AI_DECISION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
