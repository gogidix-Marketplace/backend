package com.gogidix.aiservices.aitestingservice.observability;
...
class SloComplianceTest {
    ...
    private AiTestingMetrics metrics;
    private MeterRegistry meterRegistry;

    @BeforeEach
    void setUp() {
        meterRegistry = new SimpleMeterRegistry();
        metrics = new AiTestingMetrics(meterRegistry);
    }
    ...
    class MetricsRecordingTests {
        ...
        @Test
        @Order(10)
        @DisplayName("Should record test execution latency")
        void shouldRecordTestExecutionLatency() {
            metrics.recordTestExecutionTime(100);
        ...
        @Test
        @Order(11)
        @DisplayName("Should record testing requests")
        void shouldRecordTestingRequests() {
    ...
    class SloThresholdTests {
        ...
        void shouldHaveP95LatencyBelowThreshold() {
            ...
            for (int i = 0; i < 100; i++) {
                metrics.recordTestExecutionTime(50 + i);
            }
    ...
    class MetricsStructureTests {
        ...
        @Test
        @Order(30)
        @DisplayName("Should have test execution duration timer")
        void shouldHaveTestExecutionDurationTimer() {
            assertThat(meterRegistry.get("ai.testing.test.execution.duration").timer()).isNotNull();
        }
        ...
        void shouldHaveRequestsCounter() {
            assertThat(meterRegistry.get("ai.testing.requests.total").counter()).isNotNull();
        }
    }
}
