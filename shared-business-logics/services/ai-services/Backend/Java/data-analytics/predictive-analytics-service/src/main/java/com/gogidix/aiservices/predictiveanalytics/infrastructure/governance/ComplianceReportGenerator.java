...
public class ComplianceReportGenerator {
    ...
    public ComplianceReportGenerator(
            PredictiveAnalyticsMetrics metrics,
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P99 Latency
        double p99Latency = metrics.getForecastGenerationLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("forecast.generation.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("forecast.generation.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("forecast.generation.failure").counter().count());

        section.setModelTrainings((long) metrics.getMeterRegistry()
                .get("model.training").counter().count());

        section.setPredictions((long) metrics.getMeterRegistry()
                .get("prediction").counter().count());
