...
export default function TwoFactorPage() {
  ...
  const user = useAccountantStore((s) => s.user)
  const verify2FA = useAccountantStore((s) => s.verify2FA)
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-600 via-teal-700 to-teal-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-teal-200 mt-1">Accountant Dashboard</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex justify-center gap-2" onPaste={handlePaste}>
              {code.map((digit, index) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-xl font-bold border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-colors"
                />
              ))}
            </div>
            ...
            <button
              ...
              className={`w-full py-2.5 rounded-lg font-medium text-white transition-colors text-sm ${
