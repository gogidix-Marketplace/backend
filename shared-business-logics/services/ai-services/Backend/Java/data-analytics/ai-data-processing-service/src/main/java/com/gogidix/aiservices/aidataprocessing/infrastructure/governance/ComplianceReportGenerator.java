...
public class ComplianceReportGenerator {
    ...
    private final DataProcessingMetrics metrics;
    private final ThresholdValidator thresholdValidator;

    // SLO Targets: P95 < 500ms, P99 < 1000ms, error rate < 1%
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.data.processing.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.data.processing.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.data.processing.failure").counter().count());

        section.setValidationPassed((long) metrics.getMeterRegistry()
                .get("ai.data.processing.validation.passed").counter().count());

        section.setValidationFailed((long) metrics.getMeterRegistry()
                .get("ai.data.processing.validation.failed").counter().count());

        section.setTransformationsApplied((long) metrics.getMeterRegistry()
                .get("ai.data.processing.transformation.applied").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("validationPassed", performanceMetrics.getValidationPassed());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        public long getValidationPassed() { return validationPassed; }
        public void setValidationPassed(long validationPassed) { this.validationPassed = validationPassed; }
        public long getValidationFailed() { return validationFailed; }
        public void setValidationFailed(long validationFailed) { this.validationFailed = validationFailed; }
        public long getTransformationsApplied() { return transformationsApplied; }
        public void setTransformationsApplied(long transformationsApplied) { this.transformationsApplied = transformationsApplied; }
