...
public class ComplianceReportGenerator {
    ...
    public ComplianceReportGenerator(
            ModelManagementMetrics metrics,
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.model.registration.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.model.registration.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.model.registration.failure").counter().count());

        section.setModelsLoaded((long) metrics.getMeterRegistry()
                .get("ai.model.loaded").counter().count());

        section.setModelsDeployed((long) metrics.getMeterRegistry()
                .get("ai.model.deployed").counter().count());

        section.setModelsRetired((long) metrics.getMeterRegistry()
                .get("ai.model.retired").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("modelsLoaded", performanceMetrics.getModelsLoaded());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long modelsDeployed;
        private long modelsRetired;
        ...
        public long getModelsLoaded() { return modelsLoaded; }
        public void setModelsLoaded(long modelsLoaded) { this.modelsLoaded = modelsLoaded; }
        public long getModelsDeployed() { return modelsDeployed; }
        public void setModelsDeployed(long modelsDeployed) { this.modelsDeployed = modelsDeployed; }
        public long getModelsRetired() { return modelsRetired; }
        public void setModelsRetired(long modelsRetired) { this.modelsRetired = modelsRetired; }
