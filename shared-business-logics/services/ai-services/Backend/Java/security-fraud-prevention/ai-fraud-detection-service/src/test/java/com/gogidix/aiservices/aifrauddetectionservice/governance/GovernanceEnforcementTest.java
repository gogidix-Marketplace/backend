...
class GovernanceEnforcementTest {
    ...
    @MockBean
    private com.gogidix.aiservices.aifrauddetectionservice.domain.port.out.FraudRepository fraudRepository;
    ...
    class ThresholdValidationTests {
        ...
        void shouldValidateP95LatencyThreshold() {
            doNothing().when(fraudRepository).saveAnalysisResult(any());

            // Generate some traffic
            for (int i = 0; i < 20; i++) {
                Transaction transaction = createTestTransaction("p95-validation-" + i);
                try {
        ...
        void shouldValidateErrorRateThreshold() {
            ...
            for (int i = 0; i < 10; i++) {
                Transaction transaction = createTestTransaction("error-rate-val-" + i);
                try {
                    fraudDetectionService.analyzeTransaction(transaction);
                } catch (Exception e) {
    ...
    class ComplianceReportTests {
        ...
        void shouldIncludePerformanceMetricsSection() {
            ...
            for (int i = 0; i < 5; i++) {
                try {
                    fraudDetectionService.analyzeTransaction(createTestTransaction("perf-metrics-" + i));
                } catch (Exception e) {
                    // Ignore
                }
            }
    ...
    class ThresholdEnforcementTests {
        ...
        void shouldEnforceLatencyThreshold() {
            ...
            for (int i = 0; i < requests; i++) {
                try {
                    fraudDetectionService.analyzeTransaction(createTestTransaction("enforce-latency-" + i));
                } catch (Exception e) {
                    // Ignore
                }
            }
        ...
        void shouldEnforceErrorRateThreshold() {
            ...
            for (int i = 0; i < requests; i++) {
                try {
                    fraudDetectionService.analyzeTransaction(createTestTransaction("enforce-error-" + i));
                } catch (Exception e) {
                    failures++;
                }
            }
    ...
    private Transaction createTestTransaction(String transactionId) {
        return Transaction.builder()
                ...
                .merchant("Governance Test Merchant")
