...
import com.gogidix.management.executive.alert.application.query.AlertQueryService;
...
import com.gogidix.management.executive.alert.shared.requestcontext.RequestContext;
import com.gogidix.management.executive.alert.shared.requestcontext.RequestContextHolder;
...
class AlertQueryServiceTest {
    ...
    @Mock
    private AlertRepository strategyRepository;

    @InjectMocks
    private AlertQueryService service;

    private Alert testEntity;

    @BeforeEach
    void setUp() {
        testEntity = Alert.builder()
                        .name("test-name")
            ...
            .build();
        lenient().when(strategyRepository.save(any(Alert.class))).thenAnswer(inv -> inv.getArgument(0));
    ...
    @Test
    void exists() {
        String alertId = "test-alertId";
        String tenantId = "test-tenantId";

        try {
        boolean result = service.exists(alertId, tenantId);
