...
public class ThresholdValidator {
    ...
    private static final double MAX_VALIDATION_LATENCY_MS = 200.0;

    public ThresholdValidator(DataProcessingMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate validation latency
        double validationLatency = metrics.getValidationLatencyP95();
        boolean validationCompliant = validationLatency <= MAX_VALIDATION_LATENCY_MS;
        report.addCheck("Validation Latency", validationLatency, MAX_VALIDATION_LATENCY_MS, validationCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case VALIDATION_LATENCY -> {
                double value = metrics.getValidationLatencyP95();
                boolean passed = value <= MAX_VALIDATION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_VALIDATION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
