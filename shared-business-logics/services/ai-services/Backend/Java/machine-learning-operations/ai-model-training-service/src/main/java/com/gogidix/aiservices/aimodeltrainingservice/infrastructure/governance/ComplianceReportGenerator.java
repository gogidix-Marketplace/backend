...
public class ComplianceReportGenerator {
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.model.training.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.model.training.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.model.training.failure").counter().count());

        section.setModelsTrained((long) metrics.getMeterRegistry()
                .get("ai.model.trained").counter().count());

        section.setTrainingJobsCompleted((long) metrics.getMeterRegistry()
                .get("ai.model.training.jobs.completed").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("modelsTrained", performanceMetrics.getModelsTrained());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long trainingJobsCompleted;
        ...
        public long getModelsTrained() { return modelsTrained; }
        public void setModelsTrained(long modelsTrained) { this.modelsTrained = modelsTrained; }
        public long getTrainingJobsCompleted() { return trainingJobsCompleted; }
        public void setTrainingJobsCompleted(long trainingJobsCompleted) { this.trainingJobsCompleted = trainingJobsCompleted; }
