...
public class ThresholdValidator {
    ...
    private static final double MAX_ANALYSIS_LATENCY_MS = 200.0;

    public ThresholdValidator(UserProfileationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate segment analysis latency
        double analysisLatency = metrics.getProfileAnalysisLatencyP95();
        boolean analysisCompliant = analysisLatency <= MAX_ANALYSIS_LATENCY_MS;
        report.addCheck("Profile Analysis Latency", analysisLatency, MAX_ANALYSIS_LATENCY_MS, analysisCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getProfileationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getProfileationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case SEGMENT_ANALYSIS_LATENCY -> {
                double value = metrics.getProfileAnalysisLatencyP95();
                boolean passed = value <= MAX_ANALYSIS_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_ANALYSIS_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
