...
public class ThresholdValidator {
    ...
    // Threshold definitions for Customer Experience - Search Service
    private static final double MAX_P95_LATENCY_MS = 300.0;
    private static final double MAX_P99_LATENCY_MS = 500.0;
    private static final double MAX_ERROR_RATE = 0.01;
    private static final double MAX_ENGINE_LATENCY_MS = 200.0;

    public ThresholdValidator(SearchMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double engineLatency = metrics.getSearchEngineLatencyP95();
        boolean engineCompliant = engineLatency <= MAX_ENGINE_LATENCY_MS;
        report.addCheck("Search Engine Latency", engineLatency, MAX_ENGINE_LATENCY_MS, engineCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getSearchLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getSearchLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case SEARCH_ENGINE_LATENCY -> {
                double value = metrics.getSearchEngineLatencyP95();
                boolean passed = value <= MAX_ENGINE_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_ENGINE_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
    ...
    public boolean isDegraded() {
        return !validateAllThresholds().isCompliant();
    }
    ...
    public enum ThresholdType {
        P95_LATENCY, P99_LATENCY, SEARCH_ENGINE_LATENCY, ERROR_RATE
    }
