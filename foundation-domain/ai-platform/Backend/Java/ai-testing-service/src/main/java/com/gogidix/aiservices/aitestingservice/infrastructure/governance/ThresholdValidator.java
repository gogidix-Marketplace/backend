...
public class ThresholdValidator {
    ...
    private static final double MAX_TEST_EXECUTION_LATENCY_MS = 300.0;

    public ThresholdValidator(AiTestingMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate test execution latency
        double testLatency = metrics.getTestExecutionLatencyP95();
        boolean testCompliant = testLatency <= MAX_TEST_EXECUTION_LATENCY_MS;
        report.addCheck("Test Execution Latency", testLatency, MAX_TEST_EXECUTION_LATENCY_MS, testCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getTestExecutionLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getTestExecutionLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case TEST_EXECUTION_LATENCY -> {
                double value = metrics.getTestExecutionLatencyP95();
                boolean passed = value <= MAX_TEST_EXECUTION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_TEST_EXECUTION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
