package com.gogidix.aiservices.aimonitoringservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private final AiMonitoringMetrics metrics;
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P99 Latency
        double p99Latency = metrics.getMonitoringLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.monitoring.requests.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.monitoring.requests.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.monitoring.requests.failure").counter().count());

        section.setAlertsTriggered((long) metrics.getMeterRegistry()
                .get("ai.monitoring.alerts.triggered").counter().count());

        section.setHealthChecksPerformed((long) metrics.getMeterRegistry()
                .get("ai.monitoring.healthchecks.total").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("alertsTriggered", performanceMetrics.getAlertsTriggered());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long healthChecksPerformed;
        ...
        public void setAlertsTriggered(long alertsTriggered) { this.alertsTriggered = alertsTriggered; }
        public long getHealthChecksPerformed() { return healthChecksPerformed; }
        public void setHealthChecksPerformed(long healthChecksPerformed) { this.healthChecksPerformed = healthChecksPerformed; }
