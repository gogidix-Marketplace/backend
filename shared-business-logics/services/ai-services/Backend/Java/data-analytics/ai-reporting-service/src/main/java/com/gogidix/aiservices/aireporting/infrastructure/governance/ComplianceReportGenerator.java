...
public class ComplianceReportGenerator {
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P95 Latency
        double p95Latency = metrics.getReportGenerationLatencyP95();
        ...
        // P99 Latency
        double p99Latency = metrics.getReportGenerationLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("report.generation.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("report.generation.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("report.generation.failure").counter().count());

        section.setReportDownloads((long) metrics.getMeterRegistry()
                .get("report.download").counter().count());

        section.setDataExtractions((long) metrics.getMeterRegistry()
                .get("report.data.extraction").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("reportDownloads", performanceMetrics.getReportDownloads());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long reportDownloads;
        private long dataExtractions;
        ...
        public long getReportDownloads() { return reportDownloads; }
        public void setReportDownloads(long reportDownloads) { this.reportDownloads = reportDownloads; }
        public long getDataExtractions() { return dataExtractions; }
        public void setDataExtractions(long dataExtractions) { this.dataExtractions = dataExtractions; }
