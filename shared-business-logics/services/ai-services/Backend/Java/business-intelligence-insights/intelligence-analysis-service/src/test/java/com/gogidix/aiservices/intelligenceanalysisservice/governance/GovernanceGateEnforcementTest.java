...
class GovernanceGateEnforcementTest {
    ...
    @Nested
    @DisplayName("Analysis Analysis Latency Gate")
    class AnalysisAnalysisLatencyGateTests {

        @Test
        @DisplayName("Should pass segment analysis latency gate when under threshold")
        void shouldPassAnalysisAnalysisLatencyGateWhenUnderThreshold() {
            // Record analysis latencies under 200ms threshold
            for (int i = 0; i < 30; i++) {
                var sample = metrics.startAnalysisAnalysisTimer();
                try {
