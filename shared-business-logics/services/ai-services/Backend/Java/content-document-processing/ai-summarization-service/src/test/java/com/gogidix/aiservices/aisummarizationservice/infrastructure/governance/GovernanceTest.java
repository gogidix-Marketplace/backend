...
import org.junit.jupiter.api.*; import com.gogidix.aiservices.aisummarizationservice.infrastructure.metrics.SummarizationMetrics; import io.micrometer.core.instrument.simple.SimpleMeterRegistry; import static org.assertj.core.api.Assertions.*;
class GovernanceTest {
    private ThresholdValidator v; private ComplianceReportGenerator g;
    @BeforeEach void setup() { var m = new SummarizationMetrics(new SimpleMeterRegistry()); v = new ThresholdValidator(m); g = new ComplianceReportGenerator(m, v); }
    ...
    @Test void threshold() { assertThat(v.validateThreshold(ThresholdValidator.ThresholdType.P95_LATENCY)).isNotNull(); }
    ...
    @Test void report() { assertThat(g.generateReport()).isNotNull(); }
    @Test void reportToMap() { assertThat(g.generateReport().toMap()).containsKey("reportId"); }
