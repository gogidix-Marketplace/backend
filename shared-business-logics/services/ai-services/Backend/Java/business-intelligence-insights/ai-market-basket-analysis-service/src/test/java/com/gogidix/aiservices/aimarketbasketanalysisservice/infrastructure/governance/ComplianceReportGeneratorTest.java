package com.gogidix.aiservices.aimarketbasketanalysisservice.infrastructure.governance;

import com.gogidix.aiservices.aimarketbasketanalysisservice.infrastructure.metrics.MarketBasketMetrics;
...
class ComplianceReportGeneratorTest {
    ...
    class ReportGenerationTests {
        ...
        void shouldPopulateSloComplianceSection() {
            Timer segmentationTimer = meterRegistry.timer("segmentation.duration", "service", "ai-market-basketation");
            ...
            Counter totalCounter = meterRegistry.counter("segmentation.total", "service", "ai-market-basketation");
            Counter failureCounter = meterRegistry.counter("segmentation.failure", "service", "ai-market-basketation");
        ...
        void shouldPopulatePerformanceMetricsSection() {
            Counter totalCounter = meterRegistry.counter("segmentation.total", "service", "ai-market-basketation");
            Counter successCounter = meterRegistry.counter("segmentation.success", "service", "ai-market-basketation");
            Counter failureCounter = meterRegistry.counter("segmentation.failure", "service", "ai-market-basketation");
            Counter createdCounter = meterRegistry.counter("segment.created", "service", "ai-market-basketation");
            Counter updatedCounter = meterRegistry.counter("segment.updated", "service", "ai-market-basketation");
            Counter analyzedCounter = meterRegistry.counter("customer.analyzed", "service", "ai-market-basketation");
            ...
            assertThat(perf.getTotalAnalyses()).isEqualTo(5000L);
            assertThat(perf.getSuccessfulAnalyses()).isEqualTo(4750L);
            assertThat(perf.getFailedAnalyses()).isEqualTo(250L);
            assertThat(perf.getBasketsCreated()).isEqualTo(100L);
            assertThat(perf.getBasketsUpdated()).isEqualTo(50L);
            assertThat(perf.getCustomersAnalyzed()).isEqualTo(50000L);
        }
    ...
    private void recordGoodMetrics() {
        ...
        Timer analysisTimer = meterRegistry.timer("segment.analysis.duration", "service", "ai-market-basketation");
