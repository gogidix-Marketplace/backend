...
class VendorServiceTest {
    ...
    @InjectMocks private VendorService service;
    ...
    @Test void findAll() {
        ...
        List<Vendor> result = service.findAll();
    ...
    @Test void findById_found() {
        ...
        Vendor result = service.findById("id1");
    ...
    @Test void create() {
        Vendor entity = new Vendor(); entity.setName("new");
        when(repository.save(any(Vendor.class))).thenAnswer(inv -> inv.getArgument(0));
        Vendor result = service.create(entity);
    ...
    @Test void update_found() {
        Vendor existing = createEntity();
        when(repository.findByTenantIdAndId("t1", "id1")).thenReturn(Optional.of(existing));
        when(repository.save(any(Vendor.class))).thenAnswer(inv -> inv.getArgument(0));
        Vendor updated = new Vendor(); updated.setName("updated");
        Vendor result = service.update("id1", updated);
    ...
    @Test void update_notFound() {
        when(repository.findByTenantIdAndId("t1", "missing")).thenReturn(Optional.empty());
        assertThat(service.update("missing", new Vendor())).isNull();
    }

    @Test void delete_found() {
        when(repository.findByTenantIdAndId("t1", "id1")).thenReturn(Optional.of(createEntity()));
        service.delete("id1");
        verify(repository).delete(any(Vendor.class));
    }
