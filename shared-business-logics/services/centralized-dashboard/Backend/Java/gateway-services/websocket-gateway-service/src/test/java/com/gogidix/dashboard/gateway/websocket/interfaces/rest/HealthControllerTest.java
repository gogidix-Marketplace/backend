...
class HealthControllerTest {
    ...
    void health_returnsUpStatus() {
        ...
        assertEquals("websocket-gateway-service", response.getBody().get("service"));
    }
    ...
    void info_returnsServiceInformation() {
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals("WebSocket Gateway Service", response.getBody().get("name"));
        assertEquals("1.0.0", response.getBody().get("version"));
        assertEquals(8908, response.getBody().get("port"));
    }
}
