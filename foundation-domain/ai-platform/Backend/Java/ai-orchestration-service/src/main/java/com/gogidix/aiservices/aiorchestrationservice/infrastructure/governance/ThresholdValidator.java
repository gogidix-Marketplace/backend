...
public class ThresholdValidator {
    ...
    private static final double MAX_STEP_EXECUTION_LATENCY_MS = 200.0;

    public ThresholdValidator(AiOrchestrationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate step execution latency
        double stepLatency = metrics.getStepExecutionLatencyP95();
        boolean stepCompliant = stepLatency <= MAX_STEP_EXECUTION_LATENCY_MS;
        report.addCheck("Step Execution Latency", stepLatency, MAX_STEP_EXECUTION_LATENCY_MS, stepCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getOrchestrationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getOrchestrationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case STEP_EXECUTION_LATENCY -> {
                double value = metrics.getStepExecutionLatencyP95();
                boolean passed = value <= MAX_STEP_EXECUTION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_STEP_EXECUTION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
