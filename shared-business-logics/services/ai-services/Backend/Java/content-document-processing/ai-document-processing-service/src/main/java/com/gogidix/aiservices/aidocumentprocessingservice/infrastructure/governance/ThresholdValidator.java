...
public class ThresholdValidator {
    ...
    // Threshold definitions for Content Processing - Document Processing
    ...
    private static final double MAX_OCR_LATENCY_MS = 500.0;

    public ThresholdValidator(DocumentProcessingMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate OCR processing latency
        double ocrLatency = metrics.getOcrProcessingLatencyP95();
        boolean ocrCompliant = ocrLatency <= MAX_OCR_LATENCY_MS;
        report.addCheck("OCR Processing Latency", ocrLatency, MAX_OCR_LATENCY_MS, ocrCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case OCR_PROCESSING_LATENCY -> {
                double value = metrics.getOcrProcessingLatencyP95();
                boolean passed = value <= MAX_OCR_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_OCR_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
