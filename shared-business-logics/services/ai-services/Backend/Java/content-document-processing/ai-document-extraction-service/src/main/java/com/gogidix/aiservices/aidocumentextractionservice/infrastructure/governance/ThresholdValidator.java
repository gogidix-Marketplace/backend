...
public class ThresholdValidator {
    ...
    private static final double MAX_FIELD_EXTRACTION_LATENCY_MS = 100.0;

    public ThresholdValidator(DocumentExtractionMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double fieldLatency = metrics.getFieldExtractionLatencyP95();
        boolean fieldCompliant = fieldLatency <= MAX_FIELD_EXTRACTION_LATENCY_MS;
        report.addCheck("Field Extraction Latency", fieldLatency, MAX_FIELD_EXTRACTION_LATENCY_MS, fieldCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getExtractionLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getExtractionLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case FIELD_EXTRACTION_LATENCY -> {
                double value = metrics.getFieldExtractionLatencyP95();
                boolean passed = value <= MAX_FIELD_EXTRACTION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_FIELD_EXTRACTION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
