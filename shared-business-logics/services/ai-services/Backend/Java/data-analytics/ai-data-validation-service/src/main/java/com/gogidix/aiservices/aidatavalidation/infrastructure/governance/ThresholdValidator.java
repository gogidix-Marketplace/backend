...
public class ThresholdValidator {
    ...
    // Threshold definitions for Data Analytics (from spec)
    private static final double MAX_P95_LATENCY_MS = 500.0;  // Validation
    private static final double MAX_P99_LATENCY_MS = 1000.0; // Validation
    private static final double MAX_ERROR_RATE = 0.02;        // 2%

    public ThresholdValidator(DataValidationMetrics metrics) {
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case P99_LATENCY -> {
                double value = metrics.getValidationLatencyP99();
