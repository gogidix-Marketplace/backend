package com.gogidix.aiservices.aichatbotservice.infrastructure.governance;
...
public class ThresholdValidator {
    ...
    private static final double MAX_NLU_LATENCY_MS = 300.0;

    public ThresholdValidator(ChatbotMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double p99Latency = metrics.getResponseLatencyP99();
        boolean p99Compliant = p99Latency <= MAX_P99_LATENCY_MS;
        report.addCheck("P99 Latency", p99Latency, MAX_P99_LATENCY_MS, p99Compliant);

        double nluLatency = metrics.getNluProcessingLatencyP95();
        boolean nluCompliant = nluLatency <= MAX_NLU_LATENCY_MS;
        report.addCheck("NLU Processing Latency", nluLatency, MAX_NLU_LATENCY_MS, nluCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getResponseLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getResponseLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case NLU_PROCESSING_LATENCY -> {
                double value = metrics.getNluProcessingLatencyP95();
                boolean passed = value <= MAX_NLU_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_NLU_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
