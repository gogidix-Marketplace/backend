package com.gogidix.aiservices.ainotificationservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    // SLO Targets for Notification Service
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P95 Latency
        double p95Latency = metrics.getSendNotificationLatencyP95();
        ...
        // P99 Latency
        double p99Latency = metrics.getSendNotificationLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("notification.send.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("notification.send.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("notification.send.failure").counter().count());

        section.setCancelledNotifications((long) metrics.getMeterRegistry()
                .get("notification.cancelled").counter().count());

        section.setEmailNotifications((long) metrics.getMeterRegistry()
                .get("notification.email.total").counter().count());

        section.setSmsNotifications((long) metrics.getMeterRegistry()
                .get("notification.sms.total").counter().count());

        section.setPushNotifications((long) metrics.getMeterRegistry()
                .get("notification.push.total").counter().count());

        section.setBatchProcessingRuns((long) metrics.getMeterRegistry()
                .get("notification.batch.total").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("cancelledNotifications", performanceMetrics.getCancelledNotifications());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long batchProcessingRuns;
        ...
        public long getCancelledNotifications() { return cancelledNotifications; }
        public void setCancelledNotifications(long cancelledNotifications) { this.cancelledNotifications = cancelledNotifications; }
        public long getEmailNotifications() { return emailNotifications; }
        public void setEmailNotifications(long emailNotifications) { this.emailNotifications = emailNotifications; }
        public long getSmsNotifications() { return smsNotifications; }
        public void setSmsNotifications(long smsNotifications) { this.smsNotifications = smsNotifications; }
        public long getPushNotifications() { return pushNotifications; }
        public void setPushNotifications(long pushNotifications) { this.pushNotifications = pushNotifications; }
        public long getBatchProcessingRuns() { return batchProcessingRuns; }
        public void setBatchProcessingRuns(long batchProcessingRuns) { this.batchProcessingRuns = batchProcessingRuns; }
