...
public class ComplianceReportGenerator {
    ...
    private static final double TARGET_ERROR_RATE = 0.05;  // 5%
    private static final double TARGET_AVAILABILITY = 0.99;  // 99%
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        section.setP99LatencyCompliant(p99Latency <= 10000.0);
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.training.job.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.training.job.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.training.job.failure").counter().count());

        section.setJobsStarted((long) metrics.getMeterRegistry()
                .get("ai.training.job.started").counter().count());

        section.setJobsCompleted((long) metrics.getMeterRegistry()
                .get("ai.training.job.completed").counter().count());

        section.setJobsCancelled((long) metrics.getMeterRegistry()
                .get("ai.training.job.cancelled").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("jobsStarted", performanceMetrics.getJobsStarted());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        public long getJobsStarted() { return jobsStarted; }
        public void setJobsStarted(long jobsStarted) { this.jobsStarted = jobsStarted; }
        ...
        public void setJobsCancelled(long jobsCancelled) { this.jobsCancelled = jobsCancelled; }
