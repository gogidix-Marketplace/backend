package com.gogidix.aiservices.aitrainingservice.infrastructure.governance;
...
public class ThresholdValidator {
    ...
    private static final double MAX_P95_JOB_LATENCY_MS = 5000.0;
    private static final double MAX_P99_JOB_LATENCY_MS = 10000.0;
    private static final double MAX_ERROR_RATE = 0.05;  // 5% (higher for training jobs)
    private static final double MAX_MODEL_TRAINING_LATENCY_MS = 10000.0;

    public ThresholdValidator(TrainingMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate P95 job latency
        double p95Latency = metrics.getTrainingJobLatencyP95();
        boolean p95Compliant = p95Latency <= MAX_P95_JOB_LATENCY_MS;
        report.addCheck("P95 Job Latency", p95Latency, MAX_P95_JOB_LATENCY_MS, p95Compliant);

        // Validate P99 job latency
        double p99Latency = metrics.getTrainingJobLatencyP99();
        boolean p99Compliant = p99Latency <= MAX_P99_JOB_LATENCY_MS;
        report.addCheck("P99 Job Latency", p99Latency, MAX_P99_JOB_LATENCY_MS, p99Compliant);

        // Validate model training latency
        double trainingLatency = metrics.getModelTrainingLatencyP95();
        boolean trainingCompliant = trainingLatency <= MAX_MODEL_TRAINING_LATENCY_MS;
        report.addCheck("Model Training Latency", trainingLatency, MAX_MODEL_TRAINING_LATENCY_MS, trainingCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_JOB_LATENCY -> {
                double value = metrics.getTrainingJobLatencyP95();
                boolean passed = value <= MAX_P95_JOB_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_JOB_LATENCY_MS, "ms", passed);
            }
            case P99_JOB_LATENCY -> {
                double value = metrics.getTrainingJobLatencyP99();
                boolean passed = value <= MAX_P99_JOB_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_JOB_LATENCY_MS, "ms", passed);
            }
            case MODEL_TRAINING_LATENCY -> {
