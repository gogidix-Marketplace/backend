...
public class ThresholdValidator {
    ...
    private static final double MAX_TRIGGER_LATENCY_MS = 100.0;

    public ThresholdValidator(AiWorkflowAutomationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate trigger latency
        double triggerLatency = metrics.getTriggerLatencyP95();
        boolean triggerCompliant = triggerLatency <= MAX_TRIGGER_LATENCY_MS;
        report.addCheck("Trigger Latency", triggerLatency, MAX_TRIGGER_LATENCY_MS, triggerCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case P99_LATENCY -> {
                double value = metrics.getWorkflowExecutionLatencyP99();
            ...
            case TRIGGER_LATENCY -> {
                double value = metrics.getTriggerLatencyP95();
                boolean passed = value <= MAX_TRIGGER_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_TRIGGER_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
