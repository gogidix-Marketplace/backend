...
# Development utility script for centralized-real-time-data
...
start_service() {
    echo "Starting $centralized-real-time-data..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${centralized-real-time-data}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$centralized-real-time-data started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $centralized-real-time-data..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$centralized-real-time-data stopped"
        else
            echo "$centralized-real-time-data is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $centralized-real-time-data may not be running"
    fi
...
run_tests() {
    echo "Running tests for $centralized-real-time-data..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $centralized-real-time-data in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $centralized-real-time-data health..."
...
service_info() {
    ...
    echo "==================="
    echo "Name: $centralized-real-time-data"
