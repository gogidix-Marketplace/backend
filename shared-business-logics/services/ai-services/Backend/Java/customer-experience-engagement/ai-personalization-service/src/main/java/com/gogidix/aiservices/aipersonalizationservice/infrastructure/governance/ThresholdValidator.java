package com.gogidix.aiservices.aipersonalizationservice.infrastructure.governance;
...
public class ThresholdValidator {
    ...
    private static final double MAX_PROFILE_LATENCY_MS = 200.0;

    public ThresholdValidator(PersonalizationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double p99Latency = metrics.getPersonalizationLatencyP99();
        boolean p99Compliant = p99Latency <= MAX_P99_LATENCY_MS;
        report.addCheck("P99 Latency", p99Latency, MAX_P99_LATENCY_MS, p99Compliant);

        double profileLatency = metrics.getProfileMatchingLatencyP95();
        boolean profileCompliant = profileLatency <= MAX_PROFILE_LATENCY_MS;
        report.addCheck("Profile Matching Latency", profileLatency, MAX_PROFILE_LATENCY_MS, profileCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getPersonalizationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getPersonalizationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case PROFILE_MATCHING_LATENCY -> {
                double value = metrics.getProfileMatchingLatencyP95();
                boolean passed = value <= MAX_PROFILE_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_PROFILE_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
