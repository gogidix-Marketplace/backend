...
public class ThresholdValidator {
    ...
    // Threshold definitions for Content Processing - Multimodal
    ...
    private static final double MAX_EMBEDDING_LATENCY_MS = 1000.0;

    public ThresholdValidator(MultimodalProcessingMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate embedding generation latency
        double embeddingLatency = metrics.getEmbeddingLatencyP95();
        boolean embeddingCompliant = embeddingLatency <= MAX_EMBEDDING_LATENCY_MS;
        report.addCheck("Embedding Generation Latency", embeddingLatency, MAX_EMBEDDING_LATENCY_MS, embeddingCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getProcessingLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getProcessingLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case EMBEDDING_GENERATION_LATENCY -> {
                double value = metrics.getEmbeddingLatencyP95();
                boolean passed = value <= MAX_EMBEDDING_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_EMBEDDING_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
