...
public class ThresholdValidator {
    ...
    // Threshold definitions for Business Intelligence SLOs
    ...
    private static final double MAX_PROJECT_CREATION_LATENCY_MS = 300.0;
    private static final double MAX_FINDING_PROCESSING_LATENCY_MS = 200.0;

    public ThresholdValidator(ResearchIntelligenceMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate project creation latency
        double creationLatency = metrics.getProjectCreationLatencyP95();
        boolean creationCompliant = creationLatency <= MAX_PROJECT_CREATION_LATENCY_MS;
        report.addCheck("Project Creation Latency", creationLatency, MAX_PROJECT_CREATION_LATENCY_MS, creationCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case PROJECT_CREATION_LATENCY -> {
                double value = metrics.getProjectCreationLatencyP95();
                boolean passed = value <= MAX_PROJECT_CREATION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_PROJECT_CREATION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
