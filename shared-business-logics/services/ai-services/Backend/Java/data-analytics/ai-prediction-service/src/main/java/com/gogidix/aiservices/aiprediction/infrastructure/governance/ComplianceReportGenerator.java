package com.gogidix.aiservices.aiprediction.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    // SLO Targets for Data Analytics
    private static final double TARGET_P95_LATENCY_MS = 1000.0;
    private static final double TARGET_ERROR_RATE = 0.02;  // 2%
    private static final double TARGET_AVAILABILITY = 0.98;  // 98%
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P99 Latency
        double p99Latency = metrics.getPredictionLatencyP99();
        section.setP99LatencyMs(p99Latency);
        section.setP99LatencyCompliant(p99Latency <= 2000.0);
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("prediction.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("prediction.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("prediction.failure").counter().count());

        section.setBatchPredictions((long) metrics.getMeterRegistry()
                .get("prediction.batch.total").counter().count());

        section.setHighConfidencePredictions((long) metrics.getMeterRegistry()
                .get("prediction.high.confidence").counter().count());

        section.setLowConfidencePredictions((long) metrics.getMeterRegistry()
                .get("prediction.low.confidence").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("batchPredictions", performanceMetrics.getBatchPredictions());
            map.put("highConfidencePredictions", performanceMetrics.getHighConfidencePredictions());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long batchPredictions;
        private long highConfidencePredictions;
        private long lowConfidencePredictions;
        ...
        public long getBatchPredictions() { return batchPredictions; }
        public void setBatchPredictions(long batchPredictions) { this.batchPredictions = batchPredictions; }
        public long getHighConfidencePredictions() { return highConfidencePredictions; }
        public void setHighConfidencePredictions(long highConfidencePredictions) { this.highConfidencePredictions = highConfidencePredictions; }
        public long getLowConfidencePredictions() { return lowConfidencePredictions; }
        public void setLowConfidencePredictions(long lowConfidencePredictions) { this.lowConfidencePredictions = lowConfidencePredictions; }
