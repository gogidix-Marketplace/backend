...
public class ComplianceReportGenerator {
    ...
    private final FeatureStoreMetrics metrics;
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.feature.store.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.feature.store.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.feature.store.failure").counter().count());

        section.setFeaturesStored((long) metrics.getMeterRegistry()
                .get("ai.feature.stored").counter().count());

        section.setFeaturesRetrieved((long) metrics.getMeterRegistry()
                .get("ai.feature.retrieved").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("featuresStored", performanceMetrics.getFeaturesStored());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long featuresStored;
        private long featuresRetrieved;
        ...
        public long getFeaturesStored() { return featuresStored; }
        public void setFeaturesStored(long featuresStored) { this.featuresStored = featuresStored; }
        public long getFeaturesRetrieved() { return featuresRetrieved; }
        public void setFeaturesRetrieved(long featuresRetrieved) { this.featuresRetrieved = featuresRetrieved; }
