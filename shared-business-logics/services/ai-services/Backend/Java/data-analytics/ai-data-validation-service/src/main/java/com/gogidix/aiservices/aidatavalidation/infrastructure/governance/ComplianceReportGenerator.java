package com.gogidix.aiservices.aidatavalidation.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private final DataValidationMetrics metrics;
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P99 Latency
        double p99Latency = metrics.getValidationLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("data.validation.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("data.validation.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("data.validation.failure").counter().count());

        section.setValidationsPassed((long) metrics.getMeterRegistry()
                .get("data.validation.passed").counter().count());

        section.setValidationsFailed((long) metrics.getMeterRegistry()
                .get("data.validation.failed").counter().count());

        section.setDataQualityIssues((long) metrics.getMeterRegistry()
                .get("data.validation.quality.issues").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("validationsPassed", performanceMetrics.getValidationsPassed());
            map.put("dataQualityIssues", performanceMetrics.getDataQualityIssues());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        public long getValidationsPassed() { return validationsPassed; }
        public void setValidationsPassed(long validationsPassed) { this.validationsPassed = validationsPassed; }
        public long getValidationsFailed() { return validationsFailed; }
        public void setValidationsFailed(long validationsFailed) { this.validationsFailed = validationsFailed; }
        public long getDataQualityIssues() { return dataQualityIssues; }
        public void setDataQualityIssues(long dataQualityIssues) { this.dataQualityIssues = dataQualityIssues; }
