package com.gogidix.aiservices.aiocrservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        double p99Latency = metrics.getOcrLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.ocr.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.ocr.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.ocr.failure").counter().count());

        section.setPagesProcessed((long) metrics.getMeterRegistry()
                .get("ai.ocr.pages").counter().count());

        section.setCharactersRecognized((long) metrics.getMeterRegistry()
                .get("ai.ocr.characters").counter().count());

        section.setPreprocessingFailures((long) metrics.getMeterRegistry()
                .get("ai.ocr.preprocessing.failed").counter().count());
    ...
    private String generateReportId() {
        return "GOV-OCR-" + DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss")
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("pagesProcessed", performanceMetrics.getPagesProcessed());
            map.put("charactersRecognized", performanceMetrics.getCharactersRecognized());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long charactersRecognized;
        private long preprocessingFailures;
        ...
        public long getPagesProcessed() { return pagesProcessed; }
        public void setPagesProcessed(long pagesProcessed) { this.pagesProcessed = pagesProcessed; }
        public long getCharactersRecognized() { return charactersRecognized; }
        public void setCharactersRecognized(long charactersRecognized) { this.charactersRecognized = charactersRecognized; }
        public long getPreprocessingFailures() { return preprocessingFailures; }
        public void setPreprocessingFailures(long preprocessingFailures) { this.preprocessingFailures = preprocessingFailures; }
