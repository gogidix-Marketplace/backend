# AI IntelligenceReport Analysis Service - Blueprint Certification Report
...
```
com.gogidix.aiservices.intelligenceanalysisservice
├── domain/
│   ├── model/          ← IntelligenceAnalysis, AnalysisCriteria, AnalysisType, AnalysisStatus
│   ├── repository/      ← IntelligenceAnalysisRepository interface
│   ├── policy/         ← AnalysisBusinessPolicy, IntelligenceAnalysisPolicy
│   └── port/
│       ├── in/        ← IntelligenceAnalysisServicePort (use application DTOs)
│       └── out/       ← IntelligenceAnalysisRepositoryPort (implemented by infrastructure)
├── application/
│   ├── command/        ← Create, Update, AddIntelligenceReports, RemoveIntelligenceReports, Analyze commands
│   ├── dto/           ← IntelligenceAnalysisResponseDto, PagedResponseDto, AnalysisAnalysisResponseDto
│   ├── query/          ← FindAnalysissByTenantQuery
│   └── service/        ← IntelligenceAnalysisApplicationService (orchestrates use cases)
├── infrastructure/
│   ├── persistence/     ← IntelligenceAnalysisRepositoryAdapter, IntelligenceAnalysisDocument, SpringDataIntelligenceAnalysisRepository
│   ├── messaging/       ← AnalysisEventPublisher (Kafka)
│   └── security/        ← JwtTokenProvider, JwtAuthenticationFilter, SecurityConfiguration
├── interfaces/
│   └── rest/          ← IntelligenceAnalysisController, RestExceptionHandler
└── shared/
...
| Feature | Description |
|----------|-------------|
| **IntelligenceAnalysis** | Aggregate root with lifecycle management (DRAFT → ACTIVE → INACTIVE → ARCHIVED) |
| **AnalysisCriteria** | Value object with builder pattern, supports nested criteria with logical operators |
| **AnalysisBusinessPolicy** | Business rules for segment activation, customer count updates, criteria validation |
| **IntelligenceAnalysisPolicy** | Domain policy interface defining segment business rules |
| **Repository Ports** | IntelligenceAnalysisRepositoryPort (domain contract) |

### Application Layer
| Feature | Description |
|----------|-------------|
| **Commands** | CreateAnalysisCommand, UpdateAnalysisCommand, AddIntelligenceReportsToAnalysisCommand, RemoveIntelligenceReportsFromAnalysisCommand, AnalyzeAnalysisCommand |
| **DTOs** | IntelligenceAnalysisResponseDto (immutable), PagedResponseDto, AnalysisAnalysisResponseDto |
| **Queries** | FindAnalysissByTenantQuery (pagination, filtering by type/active) |
| **Service** | IntelligenceAnalysisApplicationService - orchestrates all use cases, delegates to domain |
...
| **Messaging** | Spring Kafka | `AnalysisEventPublisher` publishes `AnalysisCreatedEvent`, `AnalysisUpdatedEvent`, `AnalysisDeletedEvent` |
...
|---------|-------------|-------|--------|
| Domain | IntelligenceAnalysisTest | 27 | ✅ |
| Domain | AnalysisCriteriaTest | 16 | ✅ |
| Application | CreateAnalysisCommandTest | 14 | ✅ |
| Application | AddIntelligenceReportsToAnalysisCommandTest | 11 | ✅ |
| Application | UpdateAnalysisCommandTest | 9 | ✅ |
| Application | FindAnalysissByTenantQueryTest | 8 | ✅ |
...
| Test | Status |
|------|--------|
| IntelligenceAnalysisIntegrationTest | Requires Docker/Testcontainers (skipped locally) |
...
spring:
  application:
    name: AI IntelligenceReport Analysis Service
  data:
    mongodb:
...
mv $SERVICE_NAME/src/main/java/com/gogidix/aiservices/intelligenceanalysisservice/AIIntelligenceAnalysisationServiceApplication.java \
      $SERVICE_NAME/src/main/java/com/gogidix/aiservices/intelligenceanalysisservice/AI${SERVICE_NAME#ai-}Application.java
