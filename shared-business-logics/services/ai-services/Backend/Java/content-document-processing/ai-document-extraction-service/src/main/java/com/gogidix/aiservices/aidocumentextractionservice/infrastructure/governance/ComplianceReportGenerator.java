package com.gogidix.aiservices.aidocumentextractionservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private final DocumentExtractionMetrics metrics;
    ...
    public ComplianceReportGenerator(
            DocumentExtractionMetrics metrics,
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        double p99Latency = metrics.getExtractionLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.document.extraction.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.document.extraction.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.document.extraction.failure").counter().count());

        section.setDocumentsProcessed((long) metrics.getMeterRegistry()
                .get("ai.document.extraction.documents").counter().count());

        section.setFieldsExtracted((long) metrics.getMeterRegistry()
                .get("ai.document.extraction.fields").counter().count());

        section.setValidationFailures((long) metrics.getMeterRegistry()
                .get("ai.document.extraction.validation.failed").counter().count());
    ...
    private String generateReportId() {
        return "GOV-DOCEXT-" + DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss")
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("fieldsExtracted", performanceMetrics.getFieldsExtracted());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        public long getFieldsExtracted() { return fieldsExtracted; }
        public void setFieldsExtracted(long fieldsExtracted) { this.fieldsExtracted = fieldsExtracted; }
