...
public class ThresholdValidator {
    ...
    // Threshold definitions for Customer Experience - Notification Service
    // P95: 200ms (fast service), P99: 500ms, Error Rate: 2%
    private static final double MAX_P95_LATENCY_MS = 200.0;
    private static final double MAX_P99_LATENCY_MS = 500.0;
    private static final double MAX_ERROR_RATE = 0.02;  // 2%
    private static final double MAX_BATCH_P95_LATENCY_MS = 5000.0;

    public ThresholdValidator(NotificationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate batch processing latency
        double batchLatency = metrics.getBatchProcessingLatencyP95();
        boolean batchCompliant = batchLatency <= MAX_BATCH_P95_LATENCY_MS;
        report.addCheck("Batch Processing P95", batchLatency, MAX_BATCH_P95_LATENCY_MS, batchCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getSendNotificationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getSendNotificationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case BATCH_PROCESSING_LATENCY -> {
                double value = metrics.getBatchProcessingLatencyP95();
                boolean passed = value <= MAX_BATCH_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_BATCH_P95_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
