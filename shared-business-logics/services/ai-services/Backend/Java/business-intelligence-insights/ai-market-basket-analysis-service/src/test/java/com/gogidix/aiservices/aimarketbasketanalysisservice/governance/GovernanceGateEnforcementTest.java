package com.gogidix.aiservices.aimarketbasketanalysisservice.governance;

import com.gogidix.aiservices.aimarketbasketanalysisservice.infrastructure.governance.ThresholdValidator;
...
class GovernanceGateEnforcementTest {
    ...
    class P95LatencyGateTests {
        ...
        void shouldPassP95LatencyGateWhenUnderThreshold() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordAnalysisTime(300 + (i * 3)); // Max ~450ms
            }
        ...
        void shouldFailP95LatencyGateWhenOverThreshold() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordAnalysisTime(400 + (i * 5)); // Max ~650ms
            }
    ...
    @Nested
    @DisplayName("Basket Analysis Latency Gate")
    class BasketAnalysisLatencyGateTests {

        @Test
        @DisplayName("Should pass segment analysis latency gate when under threshold")
        void shouldPassBasketAnalysisLatencyGateWhenUnderThreshold() {
            // Record analysis latencies under 200ms threshold
            for (int i = 0; i < 30; i++) {
                var sample = metrics.startBasketAnalysisTimer();
                try {
    ...
    class OverallHealthStatusGateTests {
        ...
        void shouldReportHealthyStatusWhenAllGatesPass() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordAnalysisTime(300); // Under P95 threshold
            }
