...
public class ThresholdValidator {
    ...
    private static final double MAX_HEALTH_CHECK_LATENCY_MS = 200.0;

    public ThresholdValidator(AiMonitoringMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate health check latency
        double healthCheckLatency = metrics.getHealthCheckLatencyP95();
        boolean healthCheckCompliant = healthCheckLatency <= MAX_HEALTH_CHECK_LATENCY_MS;
        report.addCheck("Health Check Latency", healthCheckLatency, MAX_HEALTH_CHECK_LATENCY_MS, healthCheckCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getMonitoringLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getMonitoringLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case HEALTH_CHECK_LATENCY -> {
                double value = metrics.getHealthCheckLatencyP95();
                boolean passed = value <= MAX_HEALTH_CHECK_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_HEALTH_CHECK_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
