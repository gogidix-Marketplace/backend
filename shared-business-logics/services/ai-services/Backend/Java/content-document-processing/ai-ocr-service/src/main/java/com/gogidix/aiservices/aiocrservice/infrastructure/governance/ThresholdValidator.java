...
public class ThresholdValidator {
    ...
    private static final double MAX_RECOGNITION_LATENCY_MS = 200.0;

    public ThresholdValidator(OcrMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double recognitionLatency = metrics.getRecognitionLatencyP95();
        boolean recognitionCompliant = recognitionLatency <= MAX_RECOGNITION_LATENCY_MS;
        report.addCheck("Recognition Latency", recognitionLatency, MAX_RECOGNITION_LATENCY_MS, recognitionCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getOcrLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getOcrLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case RECOGNITION_LATENCY -> {
                double value = metrics.getRecognitionLatencyP95();
                boolean passed = value <= MAX_RECOGNITION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_RECOGNITION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
