package com.gogidix.aiservices.aitestingservice.infrastructure.governance;

import com.gogidix.aiservices.aitestingservice.infrastructure.metrics.AiTestingMetrics;
...
public class ComplianceReportGenerator {
    ...
    private final AiTestingMetrics metrics;
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        // P95 Latency
        double p95Latency = metrics.getTestExecutionLatencyP95();
        ...
        // P99 Latency
        double p99Latency = metrics.getTestExecutionLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.testing.requests.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.testing.requests.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.testing.requests.failure").counter().count());

        section.setTestsExecuted((long) metrics.getMeterRegistry()
                .get("ai.testing.tests.executed").counter().count());

        section.setTestsPassed((long) metrics.getMeterRegistry()
                .get("ai.testing.tests.passed").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("testsExecuted", performanceMetrics.getTestsExecuted());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        public long getTestsExecuted() { return testsExecuted; }
        public void setTestsExecuted(long testsExecuted) { this.testsExecuted = testsExecuted; }
        public long getTestsPassed() { return testsPassed; }
        public void setTestsPassed(long testsPassed) { this.testsPassed = testsPassed; }
