...
import com.gogidix.aiservices.supplychainoptimizationservice.infrastructure.metrics.SupplyChainMetrics;
...
public class ComplianceReportGenerator {
    ...
    private final SupplyChainMetrics metrics;
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.supply.chain.optimization.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.supply.chain.optimization.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.supply.chain.optimization.failure").counter().count());

        section.setRecommendationsGenerated((long) metrics.getMeterRegistry()
                .get("ai.supply.chain.recommendations.generated").counter().count());

        section.setCostSavings(metrics.getMeterRegistry()
                .get("ai.supply.chain.cost.savings").counter().count());
