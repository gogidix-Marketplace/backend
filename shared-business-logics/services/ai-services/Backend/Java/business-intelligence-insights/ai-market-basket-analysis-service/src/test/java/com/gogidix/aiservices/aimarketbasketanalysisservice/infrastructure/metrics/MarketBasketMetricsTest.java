...
/**
 * Unit tests for MarketBasketMetrics.
 * Tests all counter and timer methods for SLO monitoring.
 */
@DisplayName("MarketBasketMetrics Tests")
class MarketBasketMetricsTest {

    private MeterRegistry meterRegistry;
    private MarketBasketMetrics metrics;

    @BeforeEach
    void setUp() {
        meterRegistry = new SimpleMeterRegistry();
        metrics = new MarketBasketMetrics(meterRegistry);
    }
    ...
    class CounterIncrementTests {
        ...
        void shouldIncrementAnalysisTotal() {
            ...
            Counter counter = meterRegistry.counter("segmentation.total", "service", "ai-market-basketation");
        ...
        void shouldIncrementAnalysisSuccess() {
            ...
            Counter counter = meterRegistry.counter("segmentation.success", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }
        ...
        void shouldIncrementAnalysisFailure() {
            ...
            Counter counter = meterRegistry.counter("segmentation.failure", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should increment segment created counter")
        void shouldIncrementBasketCreated() {
            metrics.incrementBasketCreated();

            Counter counter = meterRegistry.counter("segment.created", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should increment segment updated counter")
        void shouldIncrementBasketUpdated() {
            metrics.incrementBasketUpdated();

            Counter counter = meterRegistry.counter("segment.updated", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should increment segment deleted counter")
        void shouldIncrementBasketDeleted() {
            metrics.incrementBasketDeleted();

            Counter counter = meterRegistry.counter("segment.deleted", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }
        ...
        void shouldIncrementCustomerAnalyzed() {
            ...
            Counter counter = meterRegistry.counter("customer.analyzed", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should increment customer added to segment counter")
        void shouldIncrementCustomerAddedToBasket() {
            metrics.incrementCustomerAddedToBasket();

            Counter counter = meterRegistry.counter("customer.added.to.segment", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should increment customer removed from segment counter")
        void shouldIncrementCustomerRemovedFromBasket() {
            metrics.incrementCustomerRemovedFromBasket();

            Counter counter = meterRegistry.counter("customer.removed.from.segment", "service", "ai-market-basketation");
            assertThat(counter.count()).isGreaterThan(0);
        }
    }
    ...
    class TimerRecordingTests {
        ...
        void shouldRecordAnalysisTime() {
            ...
            Timer timer = meterRegistry.timer("segmentation.duration", "service", "ai-market-basketation");
        ...
        void shouldStartAndStopBasketAnalysisTimer() {
            Timer.Sample sample = metrics.startBasketAnalysisTimer();
            assertThat(sample).isNotNull();

            metrics.stopBasketAnalysisTimer(sample);

            Timer timer = meterRegistry.timer("segment.analysis.duration", "service", "ai-market-basketation");
            assertThat(timer.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should start and stop segment creation timer")
        void shouldStartAndStopBasketCreationTimer() {
            Timer.Sample sample = metrics.startBasketCreationTimer();
            assertThat(sample).isNotNull();

            metrics.stopBasketCreationTimer(sample);

            Timer timer = meterRegistry.timer("segment.creation.duration", "service", "ai-market-basketation");
            assertThat(timer.count()).isGreaterThan(0);
        }

        @Test
        @DisplayName("Should start and stop segment update timer")
        void shouldStartAndStopBasketUpdateTimer() {
            Timer.Sample sample = metrics.startBasketUpdateTimer();
            assertThat(sample).isNotNull();

            metrics.stopBasketUpdateTimer(sample);

            Timer timer = meterRegistry.timer("segment.update.duration", "service", "ai-market-basketation");
            assertThat(timer.count()).isGreaterThan(0);
        }
        ...
        void shouldStartAndStopCustomerProcessingTimer() {
            ...
            Timer timer = meterRegistry.timer("customer.processing.duration", "service", "ai-market-basketation");
            assertThat(timer.count()).isGreaterThan(0);
        }
    }
    ...
    class SloComplianceTests {
        ...
        void shouldGetBasketAnalysisLatencyP95() {
            Timer timer = meterRegistry.timer("segment.analysis.duration", "service", "ai-market-basketation");
            timer.record(100, TimeUnit.MILLISECONDS);
            timer.record(150, TimeUnit.MILLISECONDS);

            double p95 = metrics.getBasketAnalysisLatencyP95();
        ...
        void shouldCalculateErrorRate() {
            meterRegistry.counter("segmentation.total", "service", "ai-market-basketation").increment(100);
            meterRegistry.counter("segmentation.failure", "service", "ai-market-basketation").increment(10);
