...
public class ThresholdValidator {
    ...
    private static final double MAX_ML_INFERENCE_LATENCY_MS = 350.0;

    public ThresholdValidator(RecommendationEngineMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double mlLatency = metrics.getMlInferenceLatencyP95();
        boolean mlCompliant = mlLatency <= MAX_ML_INFERENCE_LATENCY_MS;
        report.addCheck("ML Inference Latency", mlLatency, MAX_ML_INFERENCE_LATENCY_MS, mlCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case ML_INFERENCE_LATENCY -> {
                double value = metrics.getMlInferenceLatencyP95();
                boolean passed = value <= MAX_ML_INFERENCE_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_ML_INFERENCE_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
