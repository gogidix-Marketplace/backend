package com.gogidix.aiservices.performanceoptimizationservice.infrastructure.governance;

import com.gogidix.aiservices.performanceoptimizationservice.infrastructure.metrics.PerformanceOptimizationMetrics;
...
public class ComplianceReportGenerator {
    ...
    private final PerformanceOptimizationMetrics metrics;
    ...
    public ComplianceReportGenerator(
            PerformanceOptimizationMetrics metrics,
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.performance.requests.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.performance.requests.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.performance.requests.failure").counter().count());

        section.setBottlenecksDetected((long) metrics.getMeterRegistry()
                .get("ai.performance.bottlenecks.detected").counter().count());

        section.setRecommendationsGenerated((long) metrics.getMeterRegistry()
                .get("ai.performance.recommendations.generated").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("bottlenecksDetected", performanceMetrics.getBottlenecksDetected());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long bottlenecksDetected;
        ...
        public long getBottlenecksDetected() { return bottlenecksDetected; }
        public void setBottlenecksDetected(long bottlenecksDetected) { this.bottlenecksDetected = bottlenecksDetected; }
