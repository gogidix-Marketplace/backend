package com.gogidix.aiservices.aisearchservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    public ComplianceReportGenerator(SearchMetrics metrics, ThresholdValidator thresholdValidator) {
    ...
    private SloComplianceSection generateSloComplianceSection() {
        ...
        double p99Latency = metrics.getSearchLatencyP99();
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry().get("search.total").counter().count());
        section.setSuccessfulRequests((long) metrics.getMeterRegistry().get("search.success").counter().count());
        section.setFailedRequests((long) metrics.getMeterRegistry().get("search.failure").counter().count());
        section.setSuggestionsRequests((long) metrics.getMeterRegistry().get("search.suggestions.total").counter().count());
        section.setDocumentsIndexed((long) metrics.getMeterRegistry().get("search.index.total").counter().count());
    ...
    private String generateReportId() {
        return "GOV-" + DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss").format(LocalDateTime.now(ZoneId.of("UTC")));
    }
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("suggestionsRequests", performanceMetrics.getSuggestionsRequests());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long suggestionsRequests;
        ...
        public long getSuggestionsRequests() { return suggestionsRequests; }
        public void setSuggestionsRequests(long suggestionsRequests) { this.suggestionsRequests = suggestionsRequests; }
        public long getDocumentsIndexed() { return documentsIndexed; }
        public void setDocumentsIndexed(long documentsIndexed) { this.documentsIndexed = documentsIndexed; }
