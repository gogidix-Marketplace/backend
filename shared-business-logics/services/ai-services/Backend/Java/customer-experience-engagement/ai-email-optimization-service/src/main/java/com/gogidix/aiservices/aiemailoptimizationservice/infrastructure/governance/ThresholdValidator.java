package com.gogidix.aiservices.aiemailoptimizationservice.infrastructure.governance;

import com.gogidix.aiservices.aiemailoptimizationservice.infrastructure.metrics.EmailOptimizationMetrics;
...
public class ThresholdValidator {
    ...
    private final EmailOptimizationMetrics metrics;
    ...
    private static final double MAX_AI_ANALYSIS_LATENCY_MS = 350.0;

    public ThresholdValidator(EmailOptimizationMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double p95Latency = metrics.getOptimizationLatencyP95();
        boolean p95Compliant = p95Latency <= MAX_P95_LATENCY_MS;
        report.addCheck("P95 Latency", p95Latency, MAX_P95_LATENCY_MS, p95Compliant);

        double p99Latency = metrics.getOptimizationLatencyP99();
        boolean p99Compliant = p99Latency <= MAX_P99_LATENCY_MS;
        report.addCheck("P99 Latency", p99Latency, MAX_P99_LATENCY_MS, p99Compliant);

        double aiLatency = metrics.getAiAnalysisLatencyP95();
        boolean aiCompliant = aiLatency <= MAX_AI_ANALYSIS_LATENCY_MS;
        report.addCheck("AI Analysis Latency", aiLatency, MAX_AI_ANALYSIS_LATENCY_MS, aiCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_LATENCY -> {
                double value = metrics.getOptimizationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_LATENCY -> {
                double value = metrics.getOptimizationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case AI_ANALYSIS_LATENCY -> {
                double value = metrics.getAiAnalysisLatencyP95();
                boolean passed = value <= MAX_AI_ANALYSIS_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_AI_ANALYSIS_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
