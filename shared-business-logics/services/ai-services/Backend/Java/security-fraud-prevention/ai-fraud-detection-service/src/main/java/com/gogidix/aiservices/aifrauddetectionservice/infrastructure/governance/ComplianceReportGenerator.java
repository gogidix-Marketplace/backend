...
public class ComplianceReportGenerator {
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("fraud.analysis.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("fraud.analysis.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("fraud.analysis.failure").counter().count());

        section.setFraudDetected((long) metrics.getMeterRegistry()
                .get("fraud.detected").counter().count());

        section.setHighRiskTransactions((long) metrics.getMeterRegistry()
                .get("fraud.high.risk").counter().count());

        section.setBlockedTransactions((long) metrics.getMeterRegistry()
                .get("fraud.blocked").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("fraudDetected", performanceMetrics.getFraudDetected());
            return map;
        }
