package com.gogidix.aiservices.aichurnpredictionservice.governance;

import com.gogidix.aiservices.aichurnpredictionservice.infrastructure.governance.ThresholdValidator;
...
class GovernanceGateEnforcementTest {
    ...
    class P95LatencyGateTests {
        ...
        void shouldPassP95LatencyGateWhenUnderThreshold() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordPredictionTime(300 + (i * 3)); // Max ~450ms
            }
        ...
        void shouldFailP95LatencyGateWhenOverThreshold() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordPredictionTime(400 + (i * 5)); // Max ~650ms
            }
    ...
    @Nested
    @DisplayName("Prediction Analysis Latency Gate")
    class PredictionAnalysisLatencyGateTests {

        @Test
        @DisplayName("Should pass segment analysis latency gate when under threshold")
        void shouldPassPredictionAnalysisLatencyGateWhenUnderThreshold() {
            // Record analysis latencies under 200ms threshold
            for (int i = 0; i < 30; i++) {
                var sample = metrics.startPredictionAnalysisTimer();
                try {
    ...
    class OverallHealthStatusGateTests {
        ...
        void shouldReportHealthyStatusWhenAllGatesPass() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordPredictionTime(300); // Under P95 threshold
            }
        ...
        void shouldReportDegradedStatusWhenSomeGatesFail() {
            ...
            for (int i = 0; i < 50; i++) {
                metrics.recordPredictionTime(300);
            }
