package com.gogidix.aiservices.predictiveanalytics.infrastructure.governance;
...
public class ThresholdValidator {
    ...
    private final PredictiveAnalyticsMetrics metrics;

    // Threshold definitions for Predictive Analytics Service
    private static final double MAX_P95_LATENCY_MS = 2000.0;
    private static final double MAX_P99_LATENCY_MS = 5000.0;
    private static final double MAX_PREDICTION_LATENCY_MS = 500.0;
    private static final double MAX_ERROR_RATE = 0.01;  // 1%

    public ThresholdValidator(PredictiveAnalyticsMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        // Validate P95 latency for forecast generation
        double p95GenerationLatency = metrics.getForecastGenerationLatencyP95();
        boolean p95GenerationCompliant = p95GenerationLatency <= MAX_P95_LATENCY_MS;
        report.addCheck("Forecast Generation P95 Latency", p95GenerationLatency, MAX_P95_LATENCY_MS, p95GenerationCompliant);

        // Validate P99 latency for forecast generation
        double p99GenerationLatency = metrics.getForecastGenerationLatencyP99();
        boolean p99GenerationCompliant = p99GenerationLatency <= MAX_P99_LATENCY_MS;
        report.addCheck("Forecast Generation P99 Latency", p99GenerationLatency, MAX_P99_LATENCY_MS, p99GenerationCompliant);

        // Validate prediction latency
        double predictionLatency = metrics.getPredictionLatencyP95();
        boolean predictionCompliant = predictionLatency <= MAX_PREDICTION_LATENCY_MS;
        report.addCheck("Prediction P95 Latency", predictionLatency, MAX_PREDICTION_LATENCY_MS, predictionCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            case P95_FORECAST_LATENCY -> {
                double value = metrics.getForecastGenerationLatencyP95();
                boolean passed = value <= MAX_P95_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P95_LATENCY_MS, "ms", passed);
            }
            case P99_FORECAST_LATENCY -> {
                double value = metrics.getForecastGenerationLatencyP99();
                boolean passed = value <= MAX_P99_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_P99_LATENCY_MS, "ms", passed);
            }
            case P95_PREDICTION_LATENCY -> {
                double value = metrics.getPredictionLatencyP95();
                boolean passed = value <= MAX_PREDICTION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_PREDICTION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
    ...
    public enum ThresholdType {
        P95_FORECAST_LATENCY,
        P99_FORECAST_LATENCY,
        P95_PREDICTION_LATENCY,
        ERROR_RATE
    }
