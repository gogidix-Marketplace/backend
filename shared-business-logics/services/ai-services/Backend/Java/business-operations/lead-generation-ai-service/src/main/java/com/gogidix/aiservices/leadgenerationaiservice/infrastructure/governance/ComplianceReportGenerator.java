...
public class ComplianceReportGenerator {
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("ai.lead.generation.score.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("ai.lead.generation.score.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("ai.lead.generation.score.failure").counter().count());

        section.setLeadsCreated((long) metrics.getMeterRegistry()
                .get("ai.lead.generation.created").counter().count());

        section.setLeadsConverted((long) metrics.getMeterRegistry()
                .get("ai.lead.generation.converted").counter().count());

        section.setHighQualityLeads((long) metrics.getMeterRegistry()
                .get("ai.lead.generation.high.quality").counter().count());
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("leadsCreated", performanceMetrics.getLeadsCreated());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        public long getLeadsCreated() { return leadsCreated; }
        public void setLeadsCreated(long leadsCreated) { this.leadsCreated = leadsCreated; }
        public long getLeadsConverted() { return leadsConverted; }
        public void setLeadsConverted(long leadsConverted) { this.leadsConverted = leadsConverted; }
        public long getHighQualityLeads() { return highQualityLeads; }
        public void setHighQualityLeads(long highQualityLeads) { this.highQualityLeads = highQualityLeads; }
