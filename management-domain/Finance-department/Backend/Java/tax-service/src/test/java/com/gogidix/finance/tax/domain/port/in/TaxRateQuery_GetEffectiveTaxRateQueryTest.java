package com.gogidix.finance.tax.domain.port.in;

import com.gogidix.finance.tax.domain.port.in.TaxRateQuery;
import java.math.BigDecimal;
import java.time.*;
import java.util.*;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.junit.jupiter.MockitoExtension;
import org.mockito.junit.jupiter.MockitoSettings;
import org.mockito.quality.Strictness;
import static org.junit.jupiter.api.Assertions.*;

@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
class TaxRateQuery_GetEffectiveTaxRateQueryTest {

        @Test
    void testSettersAndGetters() {
        TaxRateQuery.GetEffectiveTaxRateQuery dto = new TaxRateQuery.GetEffectiveTaxRateQuery();
        dto.setTenantId("val-tenantId");
        dto.setJurisdiction("val-jurisdiction");
        dto.setTaxType("val-taxType");
        dto.setTaxCode("val-taxCode");
        dto.setDate(LocalDate.of(2025,6,1));
        assertEquals("val-tenantId", dto.getTenantId());
        assertEquals("val-jurisdiction", dto.getJurisdiction());
        assertEquals("val-taxType", dto.getTaxType());
        assertEquals("val-taxCode", dto.getTaxCode());
        assertEquals(LocalDate.of(2025,6,1), dto.getDate());
    }

    @Test
    void testEqualsAndHashCode() {
        TaxRateQuery.GetEffectiveTaxRateQuery dto1 = new TaxRateQuery.GetEffectiveTaxRateQuery();
        TaxRateQuery.GetEffectiveTaxRateQuery dto2 = new TaxRateQuery.GetEffectiveTaxRateQuery();
        dto1.setTenantId("test");
        dto1.setJurisdiction("test");
        dto1.setTaxType("test");
        dto1.setTaxCode("test");
        dto1.setDate(LocalDate.of(2025,1,1));
        dto2.setTenantId("test");
        dto2.setJurisdiction("test");
        dto2.setTaxType("test");
        dto2.setTaxCode("test");
        dto2.setDate(LocalDate.of(2025,1,1));
        assertEquals(dto1, dto2);
        assertEquals(dto1.hashCode(), dto2.hashCode());
        dto2.setTenantId(null);
        assertNotEquals(dto1, dto2);
    }

    @Test
    void testToString() {
        TaxRateQuery.GetEffectiveTaxRateQuery dto = new TaxRateQuery.GetEffectiveTaxRateQuery();
        dto.setTenantId("test");
        dto.setJurisdiction("test");
        dto.setTaxType("test");
        dto.setTaxCode("test");
        dto.setDate(LocalDate.of(2025,1,1));
        String str = dto.toString();
        assertNotNull(str);
        assertTrue(str.length() > 0);
    }

    @Test
    void testEqualsNullAndOtherType() {
        TaxRateQuery.GetEffectiveTaxRateQuery dto = new TaxRateQuery.GetEffectiveTaxRateQuery();
        dto.setTenantId("test");
        dto.setJurisdiction("test");
        dto.setTaxType("test");
        dto.setTaxCode("test");
        dto.setDate(LocalDate.of(2025,1,1));
        assertNotEquals(null, dto);
        assertNotEquals("string", dto);
        assertEquals(dto, dto);
    }

}