...
class ThresholdValidatorTest {
    ...
    class ValidateAllThresholdsTests {
        ...
        void shouldReturnCompliantReportWhenAllThresholdsWithinLimits() {
            ...
            when(metrics.getGatewayLatencyP95()).thenReturn(400.0);  // < 500ms
            when(metrics.getGatewayLatencyP99()).thenReturn(800.0);  // < 1000ms
            when(metrics.getRoutingLatencyP95()).thenReturn(80.0);   // < 100ms
            when(metrics.getFilterLatencyP95()).thenReturn(40.0);   // < 50ms
        ...
        void shouldReturnNonCompliantWhenP95LatencyExceedsThreshold() {
            when(metrics.getGatewayLatencyP95()).thenReturn(600.0);  // > 500ms
            when(metrics.getGatewayLatencyP99()).thenReturn(800.0);
            when(metrics.getRoutingLatencyP95()).thenReturn(80.0);
            when(metrics.getFilterLatencyP95()).thenReturn(40.0);
        ...
        void shouldReturnNonCompliantWhenP99LatencyExceedsThreshold() {
            when(metrics.getGatewayLatencyP95()).thenReturn(400.0);
            when(metrics.getGatewayLatencyP99()).thenReturn(1200.0);  // > 1000ms
            when(metrics.getRoutingLatencyP95()).thenReturn(80.0);
            when(metrics.getFilterLatencyP95()).thenReturn(40.0);
            when(metrics.getErrorRate()).thenReturn(0.005);
        ...
        @Test
        @DisplayName("Should return non-compliant report when routing latency exceeds threshold")
        void shouldReturnNonCompliantWhenRoutingLatencyExceedsThreshold() {
            when(metrics.getGatewayLatencyP95()).thenReturn(400.0);
            when(metrics.getGatewayLatencyP99()).thenReturn(800.0);
            when(metrics.getRoutingLatencyP95()).thenReturn(150.0);   // > 100ms
            when(metrics.getFilterLatencyP95()).thenReturn(40.0);
            when(metrics.getErrorRate()).thenReturn(0.005);
        ...
        @Test
        @DisplayName("Should return non-compliant report when filter latency exceeds threshold")
        void shouldReturnNonCompliantWhenFilterLatencyExceedsThreshold() {
            when(metrics.getGatewayLatencyP95()).thenReturn(400.0);
            when(metrics.getGatewayLatencyP99()).thenReturn(800.0);
            when(metrics.getRoutingLatencyP95()).thenReturn(80.0);
            when(metrics.getFilterLatencyP95()).thenReturn(75.0);    // > 50ms
            when(metrics.getErrorRate()).thenReturn(0.005);
    ...
    class ValidateSpecificThresholdTests {
        ...
        void shouldValidateP95Latency() {
            when(metrics.getGatewayLatencyP95()).thenReturn(350.0);
        ...
        void shouldValidateP99Latency() {
            when(metrics.getGatewayLatencyP99()).thenReturn(850.0);
        ...
        @Test
        @DisplayName("Should validate routing latency threshold")
        void shouldValidateRoutingLatency() {
            when(metrics.getRoutingLatencyP95()).thenReturn(85.0);

            ThresholdValidator.ThresholdValidation validation =
                    validator.validateThreshold(ThresholdValidator.ThresholdType.ROUTING_LATENCY);

            assertThat(validation.getType()).isEqualTo(ThresholdValidator.ThresholdType.ROUTING_LATENCY);
            assertThat(validation.getActualValue()).isEqualTo(85.0);
            assertThat(validation.getThreshold()).isEqualTo(100.0);
            assertThat(validation.isPassed()).isTrue();
        }

        @Test
        @DisplayName("Should validate filter latency threshold")
        void shouldValidateFilterLatency() {
            when(metrics.getFilterLatencyP95()).thenReturn(45.0);

            ThresholdValidator.ThresholdValidation validation =
                    validator.validateThreshold(ThresholdValidator.ThresholdType.FILTER_LATENCY);

            assertThat(validation.getType()).isEqualTo(ThresholdValidator.ThresholdType.FILTER_LATENCY);
            assertThat(validation.getActualValue()).isEqualTo(45.0);
        ...
        void shouldDetectFailedP95Validation() {
            when(metrics.getGatewayLatencyP95()).thenReturn(550.0);
    ...
    class HealthStatusTests {
        ...
        void shouldReturnDegradedWhen75PercentPass() {
            // 4 of 5 pass = 80% > 75%
            when(metrics.getGatewayLatencyP95()).thenReturn(600.0);  // fail
            when(metrics.getGatewayLatencyP99()).thenReturn(800.0);  // pass
            when(metrics.getRoutingLatencyP95()).thenReturn(80.0);   // pass
            when(metrics.getFilterLatencyP95()).thenReturn(40.0);    // pass
        ...
        void shouldReturnUnhealthyWhenLessThan75PercentPass() {
            // 3 of 5 pass = 60% < 75%
            when(metrics.getGatewayLatencyP95()).thenReturn(600.0);   // fail
            when(metrics.getGatewayLatencyP99()).thenReturn(1200.0);  // fail
            when(metrics.getRoutingLatencyP95()).thenReturn(80.0);    // pass
            when(metrics.getFilterLatencyP95()).thenReturn(40.0);     // pass
            when(metrics.getErrorRate()).thenReturn(0.005);           // pass
        ...
        void shouldReturnUnhealthyWhenAllThresholdsFail() {
            when(metrics.getGatewayLatencyP95()).thenReturn(600.0);
            when(metrics.getGatewayLatencyP99()).thenReturn(1200.0);
            when(metrics.getRoutingLatencyP95()).thenReturn(150.0);
            when(metrics.getFilterLatencyP95()).thenReturn(75.0);
