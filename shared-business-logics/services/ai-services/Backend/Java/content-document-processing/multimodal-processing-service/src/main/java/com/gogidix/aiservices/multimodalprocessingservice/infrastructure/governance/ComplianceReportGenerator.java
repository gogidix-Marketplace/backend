package com.gogidix.aiservices.multimodalprocessingservice.infrastructure.governance;
...
public class ComplianceReportGenerator {
    ...
    private final MultimodalProcessingMetrics metrics;
    private final ThresholdValidator thresholdValidator;

    // SLO Targets for Content Processing - Multimodal
    private static final double TARGET_P95_LATENCY_MS = 2000.0;
    private static final double TARGET_P99_LATENCY_MS = 5000.0;
    ...
    public ComplianceReportGenerator(
            MultimodalProcessingMetrics metrics,
    ...
    private PerformanceMetricsSection generatePerformanceSection() {
        ...
        section.setTotalRequests((long) metrics.getMeterRegistry()
                .get("multimodal.processing.total").counter().count());

        section.setSuccessfulRequests((long) metrics.getMeterRegistry()
                .get("multimodal.processing.success").counter().count());

        section.setFailedRequests((long) metrics.getMeterRegistry()
                .get("multimodal.processing.failure").counter().count());

        section.setMultimodalProcessed((long) metrics.getMeterRegistry()
                .get("multimodal.processing.items").counter().count());

        section.setEmbeddingsGenerated((long) metrics.getMeterRegistry()
                .get("multimodal.embedding.generated").counter().count());

        section.setValidationFailures((long) metrics.getMeterRegistry()
                .get("multimodal.processing.validation.failed").counter().count());
    ...
    private String generateReportId() {
        return "GOV-MM-" + DateTimeFormatter.ofPattern("yyyyMMdd-HHmmss")
    ...
    public static class GovernanceReport {
        ...
        private Map<String, Object> performanceMetricsToMap() {
            ...
            map.put("multimodalProcessed", performanceMetrics.getMultimodalProcessed());
            map.put("embeddingsGenerated", performanceMetrics.getEmbeddingsGenerated());
            return map;
        }
    ...
    public static class PerformanceMetricsSection {
        ...
        private long multimodalProcessed;
        ...
        public long getMultimodalProcessed() { return multimodalProcessed; }
        public void setMultimodalProcessed(long multimodalProcessed) { this.multimodalProcessed = multimodalProcessed; }
        public long getEmbeddingsGenerated() { return embeddingsGenerated; }
        public void setEmbeddingsGenerated(long embeddingsGenerated) { this.embeddingsGenerated = embeddingsGenerated; }
