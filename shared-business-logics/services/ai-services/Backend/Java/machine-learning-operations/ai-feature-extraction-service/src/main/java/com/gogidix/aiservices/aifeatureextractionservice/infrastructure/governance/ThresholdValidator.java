...
public class ThresholdValidator {
    ...
    private static final double MAX_EXTRACTION_LATENCY_MS = 200.0;

    public ThresholdValidator(FeatureExtractionMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double extractionLatency = metrics.getExtractionLatencyP95();
        boolean extractionCompliant = extractionLatency <= MAX_EXTRACTION_LATENCY_MS;
        report.addCheck("Extraction Latency", extractionLatency, MAX_EXTRACTION_LATENCY_MS, extractionCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case EXTRACTION_LATENCY -> {
                double value = metrics.getExtractionLatencyP95();
                boolean passed = value <= MAX_EXTRACTION_LATENCY_MS;
                yield new ThresholdValidation(type, value, MAX_EXTRACTION_LATENCY_MS, "ms", passed);
            }
            case ERROR_RATE -> {
