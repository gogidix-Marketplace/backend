...
# Development utility script for centralized-reporting
...
start_service() {
    echo "Starting $centralized-reporting..."
    cd "$SERVICE_DIR"
    ...
    nohup java -jar target/${centralized-reporting}-*.jar > logs/app.log 2>&1 &
    echo $! > .pid
    
    echo "$centralized-reporting started with PID $(cat .pid)"
    echo "Logs available at: logs/app.log"
}

# Function to stop the service
stop_service() {
    echo "Stopping $centralized-reporting..."
    
    if [ -f .pid ]; then
        PID=$(cat .pid)
        if ps -p $PID > /dev/null; then
            kill $PID
            echo "$centralized-reporting stopped"
        else
            echo "$centralized-reporting is not running"
        fi
        rm -f .pid
    else
        echo "PID file not found. $centralized-reporting may not be running"
    fi
...
run_tests() {
    echo "Running tests for $centralized-reporting..."
    cd "$SERVICE_DIR"
...
dev_mode() {
    echo "Starting $centralized-reporting in development mode..."
    cd "$SERVICE_DIR"
    docker-compose up -d
...
health_check() {
    echo "Checking $centralized-reporting health..."
...
service_info() {
    ...
    echo "==================="
    echo "Name: $centralized-reporting"
