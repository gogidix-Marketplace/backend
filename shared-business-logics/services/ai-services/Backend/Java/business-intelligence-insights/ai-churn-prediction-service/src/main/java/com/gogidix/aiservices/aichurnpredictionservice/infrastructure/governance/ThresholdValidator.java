...
public class ThresholdValidator {
    ...
    public ThresholdValidator(ChurnPredictionMetrics metrics) {
    ...
    public ComplianceReport validateAllThresholds() {
        ...
        double analysisLatency = metrics.getPredictionAnalysisLatencyP95();
        boolean analysisCompliant = analysisLatency <= MAX_ANALYSIS_LATENCY_MS;
        report.addCheck("Prediction Analysis Latency", analysisLatency, MAX_ANALYSIS_LATENCY_MS, analysisCompliant);
    ...
    public ThresholdValidation validateThreshold(ThresholdType type) {
        return switch (type) {
            ...
            case P99_LATENCY -> {
                double value = metrics.getPredictionLatencyP99();
            ...
            case SEGMENT_ANALYSIS_LATENCY -> {
                double value = metrics.getPredictionAnalysisLatencyP95();
